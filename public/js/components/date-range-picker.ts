...
type DateRangePosition = 'bottom' | 'top';
...
class DateRangePicker {
    ...
    constructor(selector: string | HTMLInputElement, options: DateRangePickerOptions = {}) {
        const inputEl =
        ...
        if (!inputEl) {
            ...
            // Create a dummy to avoid null checks everywhere; won't be attached to DOM
            this.input = document.createElement('input');
            this.options = DateRangePicker.resolveOptions(options);
            this.startDate = null;
            this.endDate = null;
    ...
    private static resolveOptions(options: DateRangePickerOptions): ResolvedOptions {
        return {
    ...
    private buildShortcutsHTML(): string {
        return `
    ...
    private buildCalendarHTML(date: Date): string {
    ...
    private buildDaysHTML(year: number, month: number): string {
    ...
    private getPrevMonth(date: Date): Date {
        return new Date(date.getFullYear(), date.getMonth() - 1, 1);
    }
    ...
    private bindEvents(): void {
        ...
        this.dropdown.addEventListener('click', (e: MouseEvent) => {
            ...
            if (target.dataset.range) {
                this.applyShortcut(target.dataset.range as ShortcutRange);
            }
        ...
        this.dropdown.addEventListener('mouseover', (e: MouseEvent) => {
            const target = e.target as HTMLElement;
            if (
                this.startDate &&
                !this.endDate &&
                ...
                const hoverDate = new Date(target.dataset.date!);
    ...
    private showRangePreview(hoverDate: Date): void {
        ...
        days.forEach((dayEl) => {
            const day = dayEl as HTMLElement;
    ...
    private applyShortcut(range: ShortcutRange): void {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    private renderCalendars(): void {
        const leftCalendar = this.dropdown.querySelector(
            '.drp-calendar-left'
        ) as HTMLElement | null;
        const rightCalendar = this.dropdown.querySelector(
            '.drp-calendar-right'
        ) as HTMLElement | null;
    ...
    private updateFooter(): void {
        const startDisplay = this.dropdown.querySelector(
            '.drp-start-display'
        ) as HTMLElement | null;
        const endDisplay = this.dropdown.querySelector('.drp-end-display') as HTMLElement | null;
...
(window as Window).DateRangePicker = DateRangePicker;

export { DateRangePicker };
export type { DateRangePickerOptions, DateRange };
