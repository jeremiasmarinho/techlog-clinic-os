...
class ClinicService {
    ...
    static async fetchClinicInfo() {
        try {
            ...
            // Atualizar sessionStorage com dados completos
            if (data.clinic) {
     ...
     */
    static async checkPatientLimit() {
        try {
            const token = sessionStorage.getItem('token');
     ...
     */
    static showLimitAlert(type = 'user') {
        const messages = {
            user: {
     ...
     */
    static async requestUpgrade(requestedPlan, notes = '') {
        const token = sessionStorage.getItem('token');
        if (!token) {
