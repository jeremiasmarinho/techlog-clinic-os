...
test.describe.skip('Agenda Archive Functionality', () => {
    ...
    test('Confirmar deve enviar PUT request para /archive', async () => {
        ...
        const archiveRequestPromise = page.waitForRequest(
            (request) =>
                request.url().includes(`/api/leads/${appointmentId}/archive`) &&
                request.method() === 'PUT',
            { timeout: 10000 }
        );
        ...
        const archiveResponsePromise = page.waitForResponse(
            (response) =>
                response.url().includes(`/api/leads/${appointmentId}/archive`) &&
