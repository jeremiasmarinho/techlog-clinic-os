...
test.describe('Sidebar Behavior Tests', () => {
  ...
  test('Sidebar deve estar aberto por padrão e fechar ao clicar no toggle', async ({ page }) => {
    ...
    for (const pageInfo of CRM_PAGES) {
      // Navegar para a página
      await page.goto(pageInfo.url);
      await page.waitForLoadState('networkidle');
      
      // Verificar se o sidebar existe e está aberto
      ...
      console.log(`✓ ${pageInfo.name}: Sidebar aberto por padrão (width: ${sidebarBox?.width}px)`);
      ...
      console.log(`✓ ${pageInfo.name}: Sidebar fecha corretamente (width: ${sidebarBox?.width}px)`);
