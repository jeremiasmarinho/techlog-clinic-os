...
export class FinancialController {
    ...
    static dashboard = asyncHandler(async (req: Request, res: Response): Promise<void> => {
        ...
        const dashboardData = await TransactionRepository.getDashboard(clinicId);
     ...
     */
    static getFinancialReport = asyncHandler(async (req: Request, res: Response): Promise<void> => {
        const clinicId = req.clinicId;
        ...
        if (!startDate || !endDate) {
            sendValidationError(res, 'startDate e endDate são obrigatórios');
            return;
        }

        const report = await TransactionRepository.getFinancialReport(clinicId, startDate, endDate);
        res.json(report);
    });
}
