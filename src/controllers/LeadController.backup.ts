...
export class LeadController {
    ...
    static index(req: Request, res: Response): void {
        ...
        db.all(query, params, (err, rows) => {
            ...
            console.log(
                `Query returned ${rows.length} leads for view=${view}, period=${period || 'none'}, clinic=${clinicId || 'all'}`
            );
            res.json(rows);
        });
    }
    ...
    static update(req: Request, res: Response): void {
        ...
        const { status, appointment_date, doctor, notes, type, attendance_status, archive_reason } =
            result.data;
                ...
                (err, statusRows: any[]) => {
                    ...
                    db.all(
                        ...
                        (err, typeRows: any[]) => {
                            ...
                            db.all(
                                'SELECT date(created_at) as date, COUNT(*) as count FROM leads GROUP BY date(created_at) ORDER BY date(created_at) DESC LIMIT 7',
                                [],
                                (err, historyRows: any[]) => {
                                    if (err) {
                                        res.status(500).json({ error: err.message });
                                    ...
                                    db.all(
                                        'SELECT attendance_status, COUNT(*) as count FROM leads WHERE attendance_status IS NOT NULL GROUP BY attendance_status',
                                        [],
                                        (err, attendanceRows: any[]) => {
                                            if (err) {
                                                res.status(500).json({ error: err.message });
                                                return;
                                            }
