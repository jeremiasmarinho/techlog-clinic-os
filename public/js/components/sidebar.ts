...
interface CachedClinicSettings {
    settings: ClinicBrandingSettings;
    timestamp: number;
}
...
class MedicalSidebar extends HTMLElement {
    private mainContentRetries: number;
    ...
    async applyClinicBranding(): Promise<void> {
        try {
            const token =
            ...
            let settings: ClinicBrandingSettings | undefined;

            if (cached) {
                const { settings: cachedSettings, timestamp } = JSON.parse(
                    cached
                ) as CachedClinicSettings;
                const now = Date.now();
            ...
            if (!settings) {
                ...
                if (response.ok) {
                    const data = (await response.json()) as ClinicInfoResponse;
                    // Map clinic info to settings format
                    settings = {
                        identity: {
                            name: data.clinic?.name || data.name,
                            logo: data.clinic?.logo_url || undefined,
            ...
            if (settings && settings.identity) {
                // Apply logo
                const sidebarLogo = this.querySelector('#sidebar-logo') as HTMLImageElement | null;
                const logoIcon = this.querySelector('#logo-icon') as HTMLElement | null;
                ...
                if (settings.identity.name) {
                    const clinicNameEl = this.querySelector('.clinic-name') as HTMLElement | null;
                    if (clinicNameEl) {
    ...
    initSidebar(): void {
        ...
        const sidebar = this.querySelector('#sidebar') as HTMLElement | null;
        ...
        this.querySelector('#sidebarOverlay')?.addEventListener('click', () => {
            ...
            const overlay = this.querySelector('#sidebarOverlay') as HTMLElement | null;
            overlay?.classList.toggle('show');
        });

        // Setup button event listeners (for functions that need to be called)
        const dashboardBtn = this.querySelector(
        ...
        const teamBtn = this.querySelector('[onclick="openTeamModal()"]') as HTMLElement | null;
        ...
        const privacyBtn = this.querySelector(
        ...
        const refreshBtn = this.querySelector('[onclick="loadLeads()"]') as HTMLElement | null;
        if (refreshBtn && typeof window.loadLeads === 'function') {
            refreshBtn.removeAttribute('onclick');
            refreshBtn.addEventListener('click', () => window.loadLeads!());
        }

        const logoutBtn = this.querySelector('[onclick="logout()"]') as HTMLElement | null;
    ...
    updateThemeIcons(): void {
        const sunIcon = this.querySelector('#theme-icon-sun') as HTMLElement | null;
        const moonIcon = this.querySelector('#theme-icon-moon') as HTMLElement | null;
        if (sunIcon && moonIcon) {
    ...
    applyMainContentState(isExpanded: boolean): void {
