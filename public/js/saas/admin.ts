...
declare global {
    interface Window {
        SuperAdminDashboard: {
            reload: () => Promise<void>;
            loadStats: () => Promise<SystemStats | undefined>;
...
function setupEventListeners(): void {
    ...
    const searchInput = document.getElementById('searchClinics') as HTMLInputElement | null;
    if (searchInput) {
        searchInput.addEventListener('input', (e: Event) => {
            const target = e.target as HTMLInputElement;
            filterClinics(target.value);
        });
    }

    const statusFilter = document.getElementById('filterStatus') as HTMLSelectElement | null;
    if (statusFilter) {
...
async function loadSystemStats(): Promise<SystemStats | undefined> {
    try {
...
async function toggleClinicStatus(clinicId: string, currentStatus: string): Promise<void> {
...
function renderKPICards(stats: SystemStats): void {
    // MRR Card
    ...
    if (activeCard) {
        ...
        if (value) value.textContent = String(stats.clinics.active || 0);
        if (label) {
    ...
    if (growthCard) {
        ...
        const growth = (100 - churnRate).toFixed(1);
...
function renderClinicsTable(clinics: Clinic[]): void {
...
function renderActionButtons(clinic: Clinic): string {
    const isBlocked = clinic.status === 'suspended' || clinic.status === 'cancelled';
...
function attachTableEventListeners(): void {
    document.querySelectorAll<HTMLButtonElement>('.action-btn-toggle').forEach((btn) => {
        btn.addEventListener('click', () => {
            const clinicId = btn.dataset.clinicId || '';
            const currentStatus = btn.dataset.clinicStatus || '';
            toggleClinicStatus(clinicId, currentStatus);
        });
    });

    document.querySelectorAll<HTMLButtonElement>('.action-btn-impersonate').forEach((btn) => {
        btn.addEventListener('click', () => {
            const clinicId = btn.dataset.clinicId || '';
    ...
    document.querySelectorAll<HTMLButtonElement>('.action-btn-details').forEach((btn) => {
        btn.addEventListener('click', () => {
            const clinicId = btn.dataset.clinicId || '';
...
function filterClinics(searchTerm: string): void {
...
function showClinicDetails(clinicId: string): void {
