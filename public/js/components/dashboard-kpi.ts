...
class DashboardKpi extends HTMLElement {
    ...
    render(title: string, value: string, subtitle: string, icon: string, color: string): void {
        this.className =
            'glass-card rounded-xl p-6 border border-white/10 backdrop-blur-xl hover:scale-105 transition-transform';
    ...
    updateValue(newValue: string): void {
        const valueElement = this.querySelector('.kpi-value') as HTMLElement | null;
        if (valueElement) {
