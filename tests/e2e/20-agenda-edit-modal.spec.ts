...
test.describe.skip('Agenda Edit Modal - Single Step', () => {
    // TODO: These tests need appointments in the test database for the current day
    ...
    test('Modal deve preencher campos com dados do agendamento', async () => {
        ...
        let patientName = '';
        try {
            patientName = await firstCard.$eval(
                '.text-xl.font-semibold.text-slate-100, .text-white.text-xl, span.text-white',
    ...
    test('Botão Cancelar deve fechar modal sem salvar', async () => {
        ...
        const isModalClosed = await page.$eval('#editModal', (el) =>
    ...
    test('Validação de campos obrigatórios deve funcionar', async () => {
        ...
        const isNameInvalid = await page.$eval(
            '#editName',
        ...
        const isModalStillOpen = await page.$eval('#editModal', (el) =>
    ...
    test('Edição completa deve salvar dados via API', async () => {
        ...
        const patchRequestPromise = page.waitForRequest(
            (request) =>
                request.url().includes(`/api/leads/${appointmentId}`) &&
        ...
        const patchResponsePromise = page.waitForResponse(
            (response) =>
                response.url().includes(`/api/leads/${appointmentId}`) &&
        ...
        if (foundUpdatedData) {
            ...
            console.log(
                '⚠️  Updated card not visible (acceptable - may have moved to different date)'
            );
    ...
    test('Dados financeiros devem ser codificados em JSON no campo notes', async () => {
        ...
        const jsonLine = notesLines.find(
            (line: string) => line.includes('{') && line.includes('financial')
        );
